...
**agent 是唯一控制面。** 桌面读写 `GET/PUT /v1/policy`（Auto Capture、Archive All、延迟、域名列表）。扩展只拉取策略并上报 `POST /v1/sensor/heartbeat`。桌面通过 `GET /v1/events`（SSE）接收捕获/策略变化，而不是靠 1 秒轮询驱动 UI。
